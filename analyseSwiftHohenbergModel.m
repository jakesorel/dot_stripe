...
%%
m = simulate_sh_static(0.2,1e4,mask, -0.01, -0.001, "dotgradient/epsilon", 0, 1);

%%
m = simulate_sh_static(0.2,1e4,mask, -0.01, -0.001, "dotgradient/h", 1, 0);

%%
m = simulate_sh_static(0.2,1e4,mask, -0.01, -0.001, "dotgradient/h_and_epsilon", 1, 1);

%% functions

function m = simulate_sh_static(sizeFactor,Nsteps, mask,e1,h1, filenm, HPARAM, EPARAM)
rng(1);
close all
% params
N=128;
...
for t = 1:length(tspan)
    inputE = -2*(V+V.^3)-0.55;
    inputE = inputE * EPARAM;
    inputH = -0.5*V;
    inputH = inputH * HPARAM;
