...
tfin = 35000;
m = simulate_rd_growth(D, df, seed, tfin, tw, L0, Lf, W,"growth_fast", DLIMS, tplot,100);


%%
tfin = 120000;

m = simulate_rd_growth(D, df, seed, tfin, tw, L0, Lf, W, "growth_slow", DLIMS, tplot,100);
...
function m = simulate_rd_growth(D, df, seed, tfin, tw, L0, Lf, W, filenm, DLIMS, tplot,growthFactor)
...
for (i = 1:(length(tspan)))
    ...
    mask = mask > 0;
    binary(:,:,i) = mask;
    indices(:,i) = find(diff(length2(1,:) ~= 0));
end
